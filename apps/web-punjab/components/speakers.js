...
const SPEAKERS = [
    ...
    {name:"Supriya Purohit",role:"Head of Product and Growt",org:"Cosmocloud, India",image:"/images/speakers/speaker6.jpeg",socials:{
        // instagram:"#",
        // facebook:"#",
        // twitter:"#",
...
const SOCIALICONSLIST = ["instagram","twitter","facebook","github","link"]

const STRIPCOLORS = ["var(--second-color)",
  "var(--third-color)",
  "var(--fourth-color)",
  "var(--container-color)",]
...
<div class="fs_card_strip" style="background-color:${STRIPCOLORS[Math.floor(Math.random() * STRIPCOLORS.length)]};">

<div class="fs_card_social">

        ${speaker.socials && Object.keys(speaker.socials).length > 0 ? Object.keys(speaker.socials).reduce((prev,platform)=>prev + `
        <a href="${speaker.socials[platform]}" class="fs_card_social_item ${platform}" target="_blank" title="${platform.charAt(0).toUpperCase()+platform.slice(1)}">
          <span class="icon-font">
              <svg class="fs_social_icon"><use xlink:href="#icon-${SOCIALICONSLIST.includes(platform)?platform:"link"}"></use></svg>
          </span>
...
<img class="fs_image" src="${speaker.image}"/>
</div>
    <h3 class=" fs_card_title" >${speaker.name}</h3>
    <p class=" fs_card_subtitle" >${speaker.role}</p>
    <p class=" fs_card_subtitle" >${speaker.org}</p>
